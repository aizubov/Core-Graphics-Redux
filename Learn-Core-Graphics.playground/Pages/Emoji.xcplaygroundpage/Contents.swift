...
let rendered = renderer.image { ctx in
    ...
    let eye_left = CGRect(x: 300, y: 300, width: 100, height: 100)
    ctx.cgContext.addEllipse(in: eye_left)
    ctx.cgContext.drawPath(using: .fillStroke)
    
    let eye_right = CGRect(x: 600, y: 300, width: 100, height: 100)
    ctx.cgContext.addEllipse(in: eye_right)
    ctx.cgContext.drawPath(using: .fillStroke)
    ...
    let mouth = CGRect(x: 400, y: 600, width: 200, height: 100)
